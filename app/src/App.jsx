...
const App = () => {
  ...
  const fetchCache = async () => {
    const response = await fetch('http://192.168.1.105:8000/all/QUICK');
  ...
  const fetchData = async () => {
    const response = await fetch('http://192.168.1.105:8000/all/SLOW');
    const result = await response.json();
    result.sort((a,b) => b.points - a.points)
    setUsers(result);
  };
          ...
          <Accordion.Item key={index} value={item.username}>
            ...
            <Accordion.ItemContent>
              ...
              {Object.keys(item.checklists).map((checklist, index) => (
                  <Card.Root flexDirection={["column", "row"]} size = "sm" align="center"  key = {index}>
                    <Box aspectRatio="square" backgroundImage={"url(" + item[`checklists`][checklist].image + ")"} backgroundPosition="bottom" backgroundSize="cover" borderRadius={["0.375rem 0.375rem 0 0", "0.375rem 0 0 0.375rem"]}/>
                    <Card.Body>
                        <Card.Title mb="2" whiteSpace= {["wrap", "nowrap"]} overflow = "hidden" display = "block" textOverflow = "ellipsis" width={["auto", "250px"]}>{item["checklists"][checklist].bird}</Card.Title>
                        <Card.Description whiteSpace= "nowrap" overflow = "hidden" display = "block" textOverflow = "ellipsis" width={["auto", "250px"]}>{item["checklists"][checklist].location}</Card.Description>
                    </Card.Body>
