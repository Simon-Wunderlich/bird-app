...
const DesktopApp = () => {
  ...
  const submitBird = async () => {
    ...
    const response = await fetch(
      'https://flask-hello-world-tau-dusky.vercel.app/submitBird/',
      {
        method: 'POST',
  ...
  const storeImage = async () => {
    ...
    for (let i = 0; i < batches; i++) {
      const end = Math.min(
        image.length,
        Math.ceil(((i + 1) * image.length) / batches)
      );
      console.log(start);
      console.log(end);
      const batch = image.slice(start, end);
      start = end;
      const data = { img: batch };
      await fetch(
        'https://flask-hello-world-tau-dusky.vercel.app/submitImage/markiplier',
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(data),
